...
class bookinghistory(tkinter.Frame):
    def __init__(self,root):
        ...
        Label(self, image=self.TopImage).place(x=10,y=0)

        self.dockImage=PhotoImage (file="history.png") 
        Label(self, image=self.dockImage, bg="#32405b").place(x=30,y=10)
    ...
    def place_elements(self):
        ...
        cursor = con.cursor()
        cursor.execute("SELECT bikeid,models,orderid FROM orders WHERE customerid = 1111 ORDER BY orderid")
        ...
        for index, row in enumerate(data):
                ...
                self.bgframe.place(x=50,y=130+y_increment,width=1000,height=100)
                self.text = Label(self, text=f"{models}" , font=("Impact", 20))
                self.text.configure(bg="red")
                self.text.place(x=320, y=80+y_increment)
            
                self.text1 = Label(self, text="Start date ", font=("Rockwell", 10))
                self.text1.place(x=20, y=110+y_increment)
                ...
                self.label2.place(x=175, y=0)
            
                self.label3 = Label(self.bgframe, text=" DESCRIPTION ", font=("Helvetica", 15, 'bold'), fg="white", bg="#546c9c")
                self.label3.place(x=325, y=0)
            
                self.label4 = Label(self.bgframe, text="    TYPE    ", font=("Helvetica", 15, 'bold'), fg="white", bg="#546c9c")
                self.label4.place(x=475, y=0)
            
                self.return_button = Button(self.bgframe, text="RETURN", font=("Helvetica", 12, 'bold'), fg="white", bg="#32405b",command=lambda models=models: self.open_return_window(models))
                self.return_button.place(x=600, y=5)
                ...
                self.add_review_button.place(x=600, y=50)
    ...
    def open_return_window(self,model):
