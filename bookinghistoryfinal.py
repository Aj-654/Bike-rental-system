...
class bookinghistory(tkinter.Frame):
    ...
    def place_elements(self):
        ...
        cursor = con.cursor()
        print(self.uid)
        query1="SELECT customerid from customers where email=%s"
        value1=(self.uid,)
        cursor.execute(query1,value1)
        custid=cursor.fetchone()
        customerid=custid[0]
        query2="SELECT bikeid,models,orderid,price,type,status,startdate FROM orders WHERE customerid =%s ORDER BY orderid"
        value2=(customerid,)
        ...
        for index, row in enumerate(data):
                ...
                self.text1 = Label(self, text=f"{startdate}", font=("Rockwell", 10))
                ...
                self.label1 = Label(self.bgframe, text="  BIKEID    ", font=("Helvetica", 15, 'bold'), fg="white", bg="#546c9c")
                ...
                self.label3 = Label(self.bgframe, text="    TYPE    ", font=("Helvetica", 15, 'bold'), fg="white", bg="#546c9c")
                self.label3.place(x=340, y=0)
            
                self.label4 = Label(self.bgframe, text=" STATUS   ", font=("Helvetica", 15, 'bold'), fg="white", bg="#546c9c")
                ...
                self.add_review_button = Button(self.bgframe, text="REVIEW", font=("Helvetica", 12, 'bold'), fg="white", bg="#32405b",command=lambda models=models, orderids=orderids: self.open_review_window(models,orderids,bikeid))
                ...
                self.owner.place(x=85, y=45)
                
                self.kaise = Label(self.bgframe, text=f"{price}", font=("Helvetica", 15, 'bold'), fg="black", bg="#546c9c")
                self.kaise.place(x=210, y=50)
                
                self.kyuu = Label(self.bgframe, text=f"{types}", font=("Helvetica", 15, 'bold'), fg="black", bg="#546c9c")
                self.kyuu.place(x=350, y=50)
                
                self.kyuu = Label(self.bgframe, text=f"{status}", font=("Helvetica", 15, 'bold'), fg="black", bg="#546c9c")
                self.kyuu.place(x=525, y=50)
    ...
    def open_return_window(self,model,oid):
        con = mysql.connector.connect(host="localhost",database="bike_rental_database",user="root",password="root")
        cursor = con.cursor()
        cursor.execute("SELECT status FROM orders WHERE orderid = %s", (oid,))
        ...
        if status == 'ongoing':
            ...
            self.return_button = Button(self.bgframe, text="RETURN", font=("Helvetica", 12, 'bold'), fg="white", bg="#32405b",command=lambda models=models, orderids=orderids: self.return_button_click(models, orderids))
        else:
            # If status is not 'ongoing', show a message box
            messagebox.showerror("Oops", "Looks like you have already returned the bike")
    ...
    def open_review_window(self,model, oid,bikeid):
        ...
        def submit_review():
            ...
            try:
                ...
                query2="SELECT bikeid from bikes where"

                cursor.execute("INSERT INTO review (reviewid,ratings, reviews, oid, name,bikeid,customerid) VALUES (%s, %s, %s, %s, %s,%s,%s) ", (oid,rating, review, oid,model,bikeid,customerid ))
                con.commit()
                win.destroy()
